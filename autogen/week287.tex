...
You could take A to be the algebra of \emph{continuous} functions on X and
...
it will be an \emph{isomorphism} when the K&auml;hler differentials are a
finitely generated projective module.  But it won't \emph{always} be
an isomorphism!
...
v(f)(x) = v(x) f '(x_{0}) + 2(x - x_{0}) v(x) h(x) + (x - x_{0})^{2} v(h)  
